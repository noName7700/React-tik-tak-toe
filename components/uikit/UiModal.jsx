...
export function UiModal({ width = "md", className, children, isOpen = false, onClose }) {
    ...
    const modal = (
        <div onClick={handleClick} className={`fixed inset-0 bg-slate-900/60 backdrop-blur pt-10 pb-10 overflow-y-auto ${className}`}>
            <div data-id="modal" className={`flex flex-col bg-white rounded-lg min-h-[320px] mx-auto relative ${
                {
                    md: "max-w-[640px]",
                    full: "mx-5"
                }[width]}`
...
UiModal.Header = function UiModalHeader({ children, className}) {
    return (<div className={`${className} px-6 pt-6 pb-4 text-2xl`}>
        {children}
    </div>
    );
}
...
UiModal.Footer = function UiModalFooter({ children, className}) {
    return (<div className={`${className} mt-auto p-6 flex gap-4 justify-end`}>
        {children}
    </div>
    );
}
...
UiModal.Body = function UiModalBody({ children, className}) {
    return (<div className={`${className} px-6 `}>
        {children}
    </div>
    );
}
