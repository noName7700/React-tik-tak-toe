...
export const initGameState = ({ playersCount, defaultTimer, currentMoveStart }) => ({
    ...
    currentMoveStart,
    playersCount,
    timers: MOVE_ORDER.reduce((timers, symbol, idx) => {
        if (idx < playersCount) {
    ...
    switch(action.type) {
        case GAME_STATE_ACTIONS.CELL_CLICK: {
            ...
            return {
                ...
                currentMove: getNextMove(state.currentMove, state.playersCount, state.timers),
...
function updateCell(gameState, index) {
    return gameState.cells.map((cell, idx) => idx === index ? gameState.currentMove : cell);
}
